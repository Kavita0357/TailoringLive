...
<i class="fas fa-info-circle pull-left cursor-pointer" style= "margin-top: 24px; color:white" aria-hidden="true" data-toggle="popover" data-html="true" title="@lang('superadmin::lang.active_package_description')" data-placement="right" data-trigger="hover" data-content="
    <table class='table table-condensed'>
     ...
     <tr>
        <td colspan='2'>
            <i class='fa fa-check text-success'></i>
           @if(($__subscription->package_details['cloths_count'] ?? null) == null || ($__subscription->package_details['cloths_count'] ?? 0) == 0)
                @lang('superadmin::lang.unlimited')
            @else
                {{ $__subscription->package_details['cloths_count'] }}
            @endif

            @lang('superadmin::lang.cloths')
        </td>
     </tr>
     <tr>
        <td colspan='2'>
            <i class='fa fa-check text-success'></i>
            @if(($__subscription->package_details['orders_count'] ?? null) == null || ($__subscription->package_details['orders_count'] ?? 0) == 0)
                @lang('superadmin::lang.unlimited')
            @else
                {{$__subscription->package_details['orders_count']}}
            @endif

            @lang('superadmin::lang.orders')
        </td>
     </tr>
