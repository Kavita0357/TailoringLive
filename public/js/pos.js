...
$(document).ready(function () {
    ...
    $('table#pos_table tbody, table#pos_cloth_table tbody').on('click', 'i.pos_remove_row', function () {
        $(this)
            .parents('tr')
            .remove();
        pos_total_row();
    ...
    $('button#submit-sell, button#save-and-print').click(function (e) {
        ...
        if (sale_type == 'order') {
            if ($('table#pos_cloth_table tbody').find('.product_row').length <= 0) {
                toastr.warning(LANG.no_cloths_added);
                return false;
            }
        } else {
            if ($('table#pos_table tbody').find('.product_row').length <= 0) {
...
function pos_total_row() {
    ...
    $('.pos_cloth_div span.total_quantity').each(function () {
...
function calculate_billing_details(price_total) {
    ...
    if (round_off_amount != 0) {
        ...
        $('span#round_off_text').text(__currency_trans_from_en(0, true));
    }
...
function validate_discount_field() {
    ...
    if (discount_element.val() != '' && !isNaN(max_value)) {
        if (discount_type_element.val() == 'fixed') {
            var subtotal = get_subtotal() + get_cloth_subtotal();
    ...
    if ($('#sales_order_ids').length) {
        ...
        $.ajax({
            ...
            success: function (data) {
                $('#sales_order_ids').select2('destroy').empty().select2({ allowClear: true, data: data });
...
/* Order Modue */
$('#search_cloth').change(function () {
    let clothId = $(this).val();
    if (!clothId) return;

    $.ajax({
        url: `/cloths/details/${clothId}`,
        type: 'GET',
        success: function (result) {
            if (result.success) {
                add_cloth_row(result.data);
                $('#search_cloth').val(null).trigger('change'); // reset select
            } else {
                alert('Error fetching cloth details.');
                $('#search_cloth').val(null).trigger('change');
            }
        },
        error: function () {
            alert('Error fetching cloth details.');
    ...
    <tr class="product_row" data-row_index="${rowIndex}" id="cloth_row_${data.cloth.id}" data-base_price="${base_charge}">
        <td class="v-center">
            ...
            <img src="${data.cloth.cloth_image ? `/storage/${data.cloth.cloth_image}` : '/img/default.png'}" alt="product-img" loading="lazy"
                ...
                data-cloth-id="${data.cloth.id}"
                data-href="/cloths/all-details/${data.cloth.id}"
