$(document).ready(function () {
    ...
    //Orders dues datatables
    orders_payment_dues_table = $('#orders_payment_dues_table').DataTable({
        processing: true,
        serverSide: true,
        ordering: false,
        searching: false,
        ...
        ajax: {
            ...
            "data": function (d) {
                if ($('#orders_payment_dues_location').length > 0) {
                    d.location_id = $('#orders_payment_dues_location').val();
                }
            }
        },
        fnDrawCallback: function (oSettings) {
            __currency_convert_recursively($('#orders_payment_dues_table'));
        },
    });

    $('#orders_payment_dues_location').change(function () {
        orders_payment_dues_table.ajax.reload();
    });
});

function update_statistics(start, end) {
    var location_id = '';
    var tailoring_status = 'order';
    if ($('#dashboard_location').length > 0) {
        location_id = $('#dashboard_location').val();
    }
    if ($('#tailoring_status').length > 0) {
        tailoring_status = $('#tailoring_status').val();
    }
    var data = { start: start, end: end, location_id: location_id, tailoring_status: tailoring_status };
