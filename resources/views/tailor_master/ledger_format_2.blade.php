...
<div class="col-md-6 col-sm-6 col-xs-6 @if (!empty($for_pdf)) width-50 f-left @endif"
    ...
    <p><strong>{{ session('business.name') }}</strong><br>
...
<div class="col-md-6 col-sm-6 col-xs-6 @if (!empty($for_pdf)) width-50 f-right @endif"
    style="margin-top: 20px;">

    <table style="margin: auto;">
        <tr>
            <th class="text-center" style="border-top:hidden; font-size: 22px;">{{ __('lang_v1.statement') }}</th>
        </tr>
        <tr>
            <th class="text-center">@lang('lang_v1.date')</th>
        </tr>
        <tr>
            <td class="text-center">{{ $ledger_details['start_date'] }} @lang('lang_v1.to')
                {{ $ledger_details['end_date'] }}</td>
        </tr>
    </table>
</div>
<div class="col-md-12 col-sm-12 @if (!empty($for_pdf)) width-100 @endif">
        ...
        <tbody>
            @foreach ($ledger_details['ledger'] as $data)
                ...
                <tr @if (!empty($for_pdf) && $loop->iteration % 2 == 0) class="odd" @endif style="border:hidden;">
                    ...
                    <td>{{ $data['ref_no'] }} @if (!empty($data['due_date']) && $data['payment_status'] != 'paid')
                            <br>@lang('lang_v1.due') {{ @format_date($data['due_date']) }}
                        @endif
                    </td>
    ...
    <table class="table" style="margin-top: 0;">
        <tr>
            ...
            <th style="color: #2dce89 !important;font-size: 12px;">{{ strtoupper(__('lang_v1.1_30_days_past_due')) }}
            </th>
            <th style="color: #ffd026 !important;font-size: 12px;">{{ strtoupper(__('lang_v1.30_60_days_past_due')) }}
            </th>
            <th style="color: #ffa100 !important;font-size: 12px;">{{ strtoupper(__('lang_v1.60_90_days_past_due')) }}
            </th>
            <th style="color: #f5365c !important;font-size: 12px;">
                {{ strtoupper(__('lang_v1.over_90_days_past_due')) }}</th>
