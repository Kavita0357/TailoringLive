    ...
    <div class="modal-content">
        <div class="modal-header">
            ...
            <h4 class="modal-title no-print">
                ...
                @elseif(in_array($transaction->type, ['sell', 'order', 'sell_return']))
                    @lang($transaction->type == 'order' ? 'tailoring.order_no' : 'sale.invoice_no'): {{ $transaction->invoice_no }}
            ...
            <h4 class="modal-title visible-print-block">
                @if (in_array($transaction->type, ['purchase', 'expense', 'purchase_return', 'payroll']))
                    @lang('purchase.ref_no'): {{ $transaction->ref_no }}
                @elseif($transaction->type == 'sell' || $transaction->type == 'order')
                    @lang($transaction->type == 'order' ? 'tailoring.order_no' : 'sale.invoice_no'): {{ $transaction->invoice_no }}
                @elseif (in_array($transaction->type, ['hms_booking']))
                    @lang('hms::lang.booking_Id'): {{ $transaction->ref_no }}
                @endif
        ...
        <div class="modal-body">
            ...
            @else
                <div class="row invoice-info">
                    ...
                    <div class="col-sm-4 invoice-col">
                        <b>@lang($transaction->type == 'order' ? 'tailoring.order_no' : 'sale.invoice_no'):</b> #{{ $transaction->invoice_no }}<br />
                        <b>@lang('messages.date'):</b> {{ @format_date($transaction->transaction_date) }}<br />
                        <b>@lang('purchase.payment_status'):</b> {{ __('lang_v1.' . $transaction->payment_status) }}<br>
                    </div>
                </div>
            ...
            @can('send_notification')
                @if ($transaction->type == 'purchase')
                    ...
                    <br>
                @endif
                @if ($transaction->type == 'sell' || $transaction->type == 'hms_booking' || $transaction->type == 'order')
                    <div class="row no-print">
                        <div class="col-md-12 text-right">
                            <button type="button" class="tw-dw-btn tw-dw-btn-info tw-text-white tw-dw-btn-xs btn-modal"
                            ...
                            @if ($transaction->payment_status != 'paid')
                                &nbsp;
                                <button type="button"
                                    class="tw-dw-btn tw-dw-btn-warning tw-text-white tw-dw-btn-xs btn-modal"
                    ...
                    <div class="col-md-12">
                        @if (
                            (auth()->user()->can('hms.add_booking_payment') && in_array($transaction->type, ['hms_booking'])) ||
                                ...
                                (auth()->user()->can('sell.payments') && in_array($transaction->type, ['sell', 'order', 'sell_return'])) ||
                            ...
                            <a href="{{ action([\App\Http\Controllers\TransactionPaymentController::class, 'addPayment'], [$transaction->id]) }}"
                                class="tw-dw-btn tw-dw-btn-xs tw-dw-btn-outline  tw-dw-btn-primary pull-right add_payment_modal no-print"><i
                                    ...
                                    <td class="no-print" style="display: flex;">
                                        @if (
                                            (in_array($transaction->type, ['hms_booking']) && auth()->user()->can('hms.edit_booking_payment')) ||
                                                ...
                                                (auth()->user()->can('edit_sell_payment') && in_array($transaction->type, ['sell', 'order', 'sell_return'])) ||
                                                ((auth()->user()->can('all_expense.access') || auth()->user()->can('view_own_expense')) &&
                                                    $transaction->type == 'expense'))
                                            @if ($payment->method != 'advance')
                                                <button type="button"
                                                    class="tw-dw-btn tw-dw-btn-xs tw-dw-btn-outline  tw-dw-btn-info edit_payment"
                                                    data-href="{{ action([\App\Http\Controllers\TransactionPaymentController::class, 'edit'], [$payment->id]) }}"><i
                                        ...
                                        @if (
                                            (in_array($transaction->type, ['hms_booking']) && auth()->user()->can('hms.delete_booking_payment')) ||
                                                ...
                                                (auth()->user()->can('delete_sell_payment') &&
                                                    in_array($transaction->type, ['sell', 'order', 'sell_return'])) ||
                                                ((auth()->user()->can('all_expense.access') || auth()->user()->can('view_own_expense')) &&
                                                    $transaction->type == 'expense'))
                                            &nbsp; <button type="button"
                                                class="tw-dw-btn tw-dw-btn-xs tw-dw-btn-outline  tw-dw-btn-error delete_payment"
                                                data-href="{{ action([\App\Http\Controllers\TransactionPaymentController::class, 'destroy'], [$payment->id]) }}"><i
                                                    class="fa fa-trash" aria-hidden="true"></i></button>
                                        @endif
                                        &nbsp;
                                        <button type="button"
                                            class="tw-dw-btn tw-dw-btn-xs tw-dw-btn-outline  tw-dw-btn-primary view_payment"
                                        ...
                                        @if (!empty($payment->document_path))
                                            ...
                                            @if (isFileImage($payment->document_name))
                                                ...
                                                <button data-href="{{ $payment->document_path }}"
                                                    class="tw-dw-btn tw-dw-btn-xs tw-dw-btn-outline  tw-dw-btn-info view_uploaded_document"
