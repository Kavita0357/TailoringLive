<h3 class="text-muted mb-0">
    @lang('lang_v1.cogs') <span class="display_currency" data-currency_symbol="true">
        {{ $data['opening_stock'] + $data['total_purchase'] - $data['closing_stock'] }}</span>
</h3>
<small class="help-block">
    @lang('lang_v1.cogs_help_text')
</small>
<h3 class="text-muted mb-0">
    @lang('lang_v1.total_making_charge'):
    <span class="display_currency" data-currency_symbol="true">{{ $data['total_making_charge'] }}</span>
</h3>
<h3 class="text-muted mb-0">
...
<small class="help-block">
    (@lang('lang_v1.total_sell_price') + @lang('tailoring.making_charge') - @lang('lang_v1.total_purchase_price'))
...
<small class="help-block">@lang('lang_v1.gross_profit') + (@lang('lang_v1.total_sell_shipping_charge') + @lang('lang_v1.sell_additional_expense') + @lang('report.total_stock_recovered') +
    ...
    @foreach ($data['right_side_module_data'] as $module_data)
        ...
        @endif
    @endforeach
    ) <br> - ( @lang('report.total_stock_adjustment') + @lang('report.total_expense') + @lang('tailoring.total_wages') + @lang('lang_v1.total_purchase_shipping_charge') + @lang('lang_v1.total_transfer_shipping_charge') +
    @lang('lang_v1.purchase_additional_expense') + @lang('lang_v1.total_sell_discount') + @lang('lang_v1.total_reward_amount')
