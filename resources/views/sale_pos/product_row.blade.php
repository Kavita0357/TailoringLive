...
@if (isset($line_type) && $line_type == 'cloth')
    ...
    <tr class="product_row" data-row_index="{{ $row_count }}" id="cloth_row_{{ $product->cloth_id }}">
        <td>
            {{ $product->product_name ?? '' }}<br>
            {{ session('business.default_location_name') }}<br>
            {{-- <img src="{{ !empty($product->cloth_image) ? asset('storage/' . $product->cloth_image) : asset('/img/default.png') }}"
                alt="cloth-img" style="width:50px;height:auto;border-radius:5px;"><br> --}}
            <button type="button" class="btn btn-primary mt-10 style_measurement_btn"
                data-cloth-id="{{ $product->cloth_id }}"
                data-href="{{ action([\App\Http\Controllers\ClothController::class, 'getClothAllDetails'], $product->cloth_id) }}">
                {{ __('tailoring.style_measurement') }}
            </button>
        </td>
        ...
        <td>
            <input class="form-control input_number row_discount_amount"
                ...
                value="{{ $discount }}"><br>
            <select class="form-control row_discount_type" name="cloths[{{ $row_count }}][line_discount_type]">
        ...
        <td>
            ...
            {{-- @if (!empty($product->media))
                <img src="@if (count($product->media) > 0) {{ $product->media->first()->display_url }}
                    ...
                    alt="product-img"
                    loading="lazy"style="height: 100%;display: inline;margin-left: 3px; border: black;border-radius: 5px; margin-top: 5px; width: 50px;object-fit: cover;">
            @endif --}}
            ...
            @php
                $warranty_id = null;
                if (!empty($action) && $action == 'edit' && isset($product->warranties)) {
                    $first_warranty = collect($product->warranties)->first();
                    $warranty_id = $first_warranty->id ?? ($product->warranty_id ?? null);
                } else {
            ...
            @if (empty($is_direct_sell))
                <div class="modal fade row_edit_product_price_model"
                    id="row_edit_product_price_modal_{{ $row_count }}" tabindex="-1" role="dialog">
