<table class="table @if (!empty($for_ledger)) table-slim mb-0 bg-light-gray @else bg-gray @endif"
    @if (!empty($for_pdf)) style="width: 100%;" @endif>
    <tr @if (empty($for_ledger)) class="bg-green" @endif>
        <th>#</th>
        <th>{{ __('tailoring.cloth') }}</th>
    ...
    @foreach ($sell->sell_lines as $sell_line)
        @php
            $is_cloth_line = $sell_line->cloth_id;
        @endphp
        @if ($is_cloth_line)
            <tr>
                <td>{{ $i + 1 }}</td>
                ...
                <td>
                    {{ @format_quantity($sell_line->completed_quantity) }}
                </td>
                <td>
