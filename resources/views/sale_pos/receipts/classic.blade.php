...
@php
    $type = $receipt_details->type;
@endphp
<div style="width: 4.5in; max-width: 100%; margin: 0 auto;">
    <div class="row" style="color: #000000 !important;">
        <!-- Logo -->
        @if (empty($receipt_details->letter_head))
            {{-- @if (!empty($receipt_details->logo))
                <img style="max-height: 120px; width: auto;"
                    src="{{ url('uploads/business_logos/' . $receipt_details->logo) }}"
    ...
    <div class="col-xs-12" style="border-top: 1px solid #ddd;">
        ...
        <p style="width: 100% !important; padding-top:10px" class="word-wrap">
            ...
            <span class="pull-right text-right">
                ...
                @if ($type == 'order')
                    <b>@lang('tailoring.order_date')</b> {{ $receipt_details->invoice_date }}
                    <br><b>@lang('tailoring.delivery_date')</b> {{ $receipt_details->delivery_date }}
                @else
...
<div class="row" style="color: #000000 !important;">
    <div class="col-xs-12">
        ...
        @endif
        @php
            $total = collect($receipt_details->lines)
                ->filter(function ($line) {
                    return empty($line['line_type']) || $line['line_type'] !== 'order';
                })
                ->count();
        @endphp
        @if ($total > 0)
