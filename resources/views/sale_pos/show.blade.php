<div class="modal-dialog modal-xl no-print" role="document">
    <div class="modal-content">
        <div class="modal-header">
            ...
            <h4 class="modal-title" id="modalTitle"> @lang($sell->type == 'order' ? 'tailoring.order_details' : 'sale.sell_details') (<b>
                    @if ($sell->type == 'sales_order')
                        @lang('restaurant.order_no')
                    @else
                        @lang($sell->type == 'order' ? 'tailoring.order_no' : 'sale.invoice_no')
                    @endif :
                </b> {{ $sell->invoice_no }})
            </h4>
        </div>
        <div class="modal-body">
            @if ($sell->type != 'order')
                <div class="row">
                    <div class="col-xs-12">
                        <p class="pull-right"><b>@lang('messages.date'):</b>
            ...
            <div class="row">
                ...
                @endphp
                <div class="@if (!empty($export_custom_fields) || $sell->type == 'order') col-sm-3 @else col-sm-4 @endif">
                    <b>
                        @if ($sell->type == 'sales_order')
                ...
                <div class="@if (!empty($export_custom_fields) || $sell->type == 'order') col-sm-3 @else col-sm-4 @endif">
                    ...
                    <strong>@lang($sell->type == 'order' ? 'tailoring.order_date' : 'sale.shipping'):</strong>
                    @if ($sell->type == 'order')
                        <span>{{ @format_date($sell->transaction_date) }}</span>
                    @else
                        <span
                ...
                @if ($sell->type == 'order')
                    <div class="col-sm-3">
                        <strong>@lang('tailoring.delivery_date'):</strong>
                           {{ @format_date($sell->delivery_date) }}
                    </div>
                @endif
            ...
            @php
                $product_count = $sell->sell_lines->whereNull('cloth_id')->count();
            @endphp
            <br>
            <div class="row">
                @if ($sell->type == 'order')
                    <div class="col-sm-12 col-xs-12">
                        <h4>{{ __('tailoring.cloths') }}:</h4>
                    </div>
                    <div class="col-sm-12 col-xs-12">
                        <div class="table-responsive">
                            @include('sale_pos.partials.cloth_line_details')
                        </div>
                    </div>
                @endif
                @if ($product_count > 0)
                    ...
                    <div class="col-sm-12 col-xs-12">
                        <div class="table-responsive">
                            @include('sale_pos.partials.product_sale_line_details')
                        </div>
                    </div>
                @endif
            </div>
            ...
            <div class="row">
                <div class="col-sm-6">
                    <strong>{{ $sell->type == 'order' ? __('tailoring.order_note') : __('sale.sell_note') }}:</strong><br>
        ...
        <div class="modal-footer">
            @if ($sell->type != 'sales_order' && $sell->type != 'order')
            ...
            @endif
            @can('print_invoice')
                <a href="#" class="print-invoice tw-dw-btn tw-dw-btn-primary tw-text-white"
