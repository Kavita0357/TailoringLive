    ...
    <div style="border: 1px solid #000; padding: 10px;">
        ...
        <table
            ...
            @if ($contact->type == 'customer' || $contact->type == 'both')
                <tr>
                    <td>@lang('tailoring.total_orders')</td>
                    <td class="align-right">@format_currency($ledger_details['all_order_total_invoice'])</td>
                </tr>
            @endif
            ...
            <tbody>
                @foreach ($ledger_details['ledger'] as $data)
                    ...
                    <tr @if (!empty($for_pdf) && $loop->iteration % 2 == 0) class="odd" @endif>
                        ...
                        <td>{{ $data['type'] }}</td>
                        <td>{{ $data['location'] }}</td>
                        <td>{{ isset($data['payment_status']) && !empty($data['payment_status']) ? $data['payment_status'] : '' }}
                        </td>
