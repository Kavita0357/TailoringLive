...
@section('content')
                ...
                <div class="nav-tabs-custom">
                    <ul class="nav nav-tabs nav-justified">
                        ...
                        @if (in_array('tailoring', $enabled_modules))
                            <li class="
                                @if (!empty($view_type) && $view_type == 'style_measurements') active @endif">
                                <a href="#style_measurement_tab" data-toggle="tab" aria-expanded="true">
                                    <i class="fas fa-tshirt" aria-hidden="true"></i> @lang('tailoring.style_measurement2')</a>
                            </li>
                        @endif
                        @if (in_array('tailoring', $enabled_modules))
                            <li class="
                                @if (!empty($view_type) && $view_type == 'order_list') active @endif">
                                <a href="#order_list_tab" data-toggle="tab" aria-expanded="true">
                                    <i class="fas fa-shopping-cart"></i> @lang('tailoring.order_list')</a>
                            </li>
                        @endif
                    ...
                    <div class="tab-content">
                        ...
                        @if (in_array($contact->type, ['both', 'customer']))
                            <div class="tab-pane 
                            @if (!empty($view_type) && $view_type == 'sales') active @endif"
                                id="style_measurement_tab">
                                <div class="row">
                                    <div class="col-md-12">
                                        @component('components.widget')
                                            @include('sell.partials.sell_list_filters', [
                                ...
                                <div class="row">
                                    <div class="col-md-12">
                                        <div class="style-measurement-wrapper"></div>
                                    </div>
                                </div>
                            </div>
                            @if (in_array('subscription', $enabled_modules))
    ...
    <section class="print_section" id="style_measurement_print_section"></section>
...
@section('javascript')
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $('a[href="#sales_tab"]').on('shown.bs.tab', function(e) {
                ...
                sell_table.columns.adjust().draw();
            });
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            const currentClothId = $(document).find('#select_cloth').val();
            if (currentClothId) {
                const customerId = {{ $contact->id }};
                const url = `/cloths/all-details/${currentClothId}?contact_id=${customerId}&view_only=true`;
                getClothData(url);
            }
            ...
            $(document).on('change', '#select_cloth', function() {
                const clothId = $(this).val();
                const customerId = {{ $contact->id }};
                const url = `/cloths/all-details/${clothId}?contact_id=${customerId}&view_only=true`;
                getClothData(url)
            });

            $(document).on('click', '#style_measurement_print_btn', function() {
                const clothId = $(document).find('#select_cloth').val();
                const customerId = {{ $contact->id }};
                const url =
                    `/cloths/all-details/${clothId}?contact_id=${customerId}&view_only=true&print_only=true`;
                getClothData(url, true)
            });

            function getClothData(url, isPrint = false) {
                $.ajax({
                    method: 'GET',
                    url: url,
                    success: function(result) {
                        if (result) {
                            if (isPrint) {
                                $('#style_measurement_print_section').html(result);
                                var title = document.title;
                                __print_receipt('style_measurement_print_section');
                                setTimeout(function() {
                                    document.title = title;
                                }, 1200);
                            } else
                                $(document).find('.style-measurement-wrapper').empty().append(result);
                        } else {
