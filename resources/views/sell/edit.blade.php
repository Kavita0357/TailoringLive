...
@php
    ...
    if ($transaction->type == 'order') {
        if (!empty($transaction->status) && $transaction->status == 'draft') {
            $title = __('tailoring.edit_draft_order');
        } else {
            $title = __('tailoring.edit_order');
        }
    }
    $filtered_statuses = $transaction->type == 'order' ? collect($statuses)->except('proforma')->toArray() : $statuses;
@endphp
@section('title', $title)
...
@section('content')
    ...
    <section class="content-header">
        <h1 class="tw-text-xl md:tw-text-3xl tw-font-bold tw-text-black">{{ $title }} <small
                class="tw-text-sm md:tw-text-base tw-text-gray-700 tw-font-semibold">(@if ($transaction->type == 'sales_order')
    ...
    <section class="content">
            ...
            <div class="col-md-12 col-sm-12">
                @component('components.widget', ['class' => 'box-solid'])
                    ...
                    <div class="@if (!empty($commission_agent)) col-sm-3 @else col-sm-4 @endif">
                        <div class="form-group">
                            {!! Form::label('contact_id', __('tailoring.select_customer') . ':*') !!}
                            <div class="input-group">
                                <span class="input-group-addon">
                    ...
                    <div class="col-sm-4">
                        @if ($transaction->type == 'order')
                            <div class="form-group">
                                {!! Form::label('transaction_date', __('tailoring.order_date') . ':*') !!}
                                <div class="input-group">
                                    <span class="input-group-addon">
                        ...
                        @if ($transaction->type == 'order')
                            <div class="form-group">
                                ...
                                <div class="input-group">
                                    ...
                                    {!! Form::text('delivery_date', $transaction->delivery_date, [
                                        'class' => 'form-control',
                                        'readonly',
                                        'required',
                                    ]) !!}
                    ...
                    @endif
                    @if ($transaction->type != 'sales_order' && $transaction->type != 'order')
                        <div class="@if (!empty($commission_agent)) col-sm-3 @else col-sm-4 @endif">
                            <div class="form-group">
                    ...
                    @else
                        <div class="@if (!empty($commission_agent)) col-sm-3 @else col-sm-4 @endif">
                            <div class="form-group">
                                ...
                                {!! Form::select('status', $filtered_statuses, $status, [
                                    'class' => 'form-control select2',
                                    'placeholder' => __('messages.please_select'),
                                    'required',
                                ]) !!}
                    ...
                    @component('components.widget', ['class' => 'box-solid'])
                        <div class="col-sm-12">
                            <div class="form-group tw-flex tw-items-center search-box">
                                {!! Form::label('search_cloth', __('tailoring.add_cloth') . ':*') !!}
                                {!! Form::select('search_cloth', $cloths, null, [
                                    'id' => 'search_cloth',
                                    'class' => 'form-control select select2 mousetrap',
                                    'placeholder' => __('tailoring.select_cloths'),
                                    'multiple' => false,
                                    'required' => false,
                                ]) !!}
                                {{-- <div class="input-group">
                                    {!! Form::text('search_cloth', null, [
                                        'class' => 'form-control mousetrap',
                                        'id' => 'search_cloth',
                                        'placeholder' => __('tailoring.search_cloth_placeholder'),
                                        'disabled' => is_null($default_location) ? true : false,
                                        'autofocus' => is_null($default_location) ? false : true,
                                    ]) !!}
                                    <span class="input-group-btn">
                                        <button type="button" class="btn btn-default bg-white btn-flat pos_add_quick_cloth"
                                            data-href="{{ action([\App\Http\Controllers\ClothController::class, 'create']) }}"
                                            data-container=".quick_add_cloth_modal"><i
                        ...
                        <div class="row col-sm-12 pos_cloth_div" style="min-height: 0">
                                        ...
                                        @foreach ($sell_details as $sell_line)
                                            @if ($sell_line->cloth_id != null)
                                                @include('sale_pos.product_row', [
                                                    ...
                                                    'action' => 'edit',
                                                    'is_direct_sell' => true,
                                                    'so_line' => isset($sell_line->product_id)
                                                        ? $sell_line->so_line ?? null
                                                        : null,
                            ...
                            <div class="table-responsive">
                                <table class="table table-condensed table-bordered table-striped" id="cloth_price">
                                    <tr>
                                        <td>
                ...
                @component('components.widget', ['class' => 'box-solid'])
                    <div class="col-sm-12">
                        <div class="form-group tw-flex tw-items-center search-box-fabric">
                            @if ($transaction->type == 'order')
                                {!! Form::label('search_product', __('tailoring.add_fabrics') . ':*') !!}
                            @endif
                            <div class="input-group">
        ...
        @if ($transaction->type == 'sell' || $transaction->type == 'order')
            @can('sell.payments')
                @component('components.widget', ['class' => 'box-solid bg-lightgray', 'title' => __('purchase.add_payment')])
                    <div class="row">
    ...
    <!-- quick cloth modal -->
    <div class="modal fade quick_add_cloth_modal" tabindex="-1" role="dialog" aria-labelledby="modalTitle"></div>
    <!-- quick product modal -->
    <div class="modal fade quick_add_product_modal" tabindex="-1" role="dialog" aria-labelledby="modalTitle"></div>
    <div class="modal fade style_measurement_modal" tabindex="-1" role="dialog"
        ...
        $(document).ready(function() {
            ...
            $('#status').change(function() {
                if ($(this).val() == 'final') {
                    ...
                    $('div.delivery_status').removeClass('hide');
                } else {
                    $('#payment_rows_div').addClass('hide');
                    $('div.delivery_status').addClass('hide');
                }
            });
