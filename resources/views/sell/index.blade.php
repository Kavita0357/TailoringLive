@extends('layouts.app')
@php
    $sale_type = request()->is('cloth-orders') ? 'order' : 'sell';
    $title = __('lang_v1.all_sales');
    $sub_title = __('sale.sells');
    if ($sale_type == 'order') {
        $title = __('tailoring.all_orders');
        $sub_title = __('tailoring.orders');
    }
@endphp
@section('title', $title)
...
@section('content')
    ...
    <section class="content-header no-print">
        <h1 class="tw-text-xl md:tw-text-3xl tw-font-bold tw-text-black">{{ $sub_title }}
        </h1>
    </section>
    ...
    <section class="content no-print">
        @component('components.filters', ['title' => __('report.filters')])
            ...
            @if (!empty($sources) && $sale_type != 'order')
                <div class="col-md-3">
                    <div class="form-group">
            ...
            @endif
        @endcomponent
        @component('components.widget', ['class' => 'box-primary', 'title' => $title])
            @can('direct_sell.access')
                @slot('tool')
                    <div class="box-tools">
                        <a class="tw-dw-btn tw-bg-gradient-to-r tw-from-indigo-600 tw-to-blue-500 tw-font-bold tw-text-white tw-border-none tw-rounded-full pull-right"
                            href="{{ $sale_type == 'order' ? route('cloth_orders.create') : route('sells.create') }}">
                ...
                <table class="table table-bordered table-striped ajax_view" id="sell_table">
                    <thead>
                        <tr>
                            ...
                            <th>@lang($sale_type == 'order' ? 'tailoring.order_date' : 'messages.date')</th>
                            <th>@lang($sale_type == 'order' ? 'tailoring.order_no' : 'sale.invoice_no')</th>
                            ...
                            <th>@lang($sale_type == 'order' ? 'tailoring.order_note' : 'sale.sell_note')</th>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            sell_table = $('#sell_table').DataTable({
                ...
                "ajax": {
                    "url": saleType == 'order' ? '/cloth-orders' : "/sells",
                    "data": function(d) {
                ...
                columns: [{
                    ...
                    },
                    {
                        ...
                        name: 'tos.name',
                        @if (empty($is_types_service_enabled) || $sale_type == 'order')
                            visible: false,
                        @endif
                    },
                    {
                    ...
                    },
                    {
                        ...
                        @if (empty($custom_labels['sell']['custom_field_1']) || $sale_type == 'order')
                            visible: false
                        @endif
                    },
                    {
                        ...
                        @if (empty($custom_labels['sell']['custom_field_2']) || $sale_type == 'order')
                            visible: false
                        @endif
                    },
                    {
                        ...
                        @if (empty($custom_labels['sell']['custom_field_3']) || $sale_type == 'order')
                            visible: false
                        @endif
                    },
                    {
                        ...
                        @if (empty($custom_labels['sell']['custom_field_4']) || $sale_type == 'order')
                            visible: false
                        @endif
                    },
                    {
