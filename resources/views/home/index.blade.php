...
@section('content')
    ...
    <div
        ...
        <div class="tw-px-5 tw-pt-3">
            ...
            @if (auth()->user()->can('dashboard.data'))
                @if ($is_admin)
                    ...
                    <div
                        class="tw-grid tw-grid-cols-1 tw-gap-4 tw-mt-6 sm:tw-grid-cols-2 xl:tw-grid-cols-4 sm:tw-gap-5 sell-summary hide">
                        ...
                        <div
                            ...
                            <div class="tw-p-4 sm:tw-p-5">
                                <div class="tw-flex tw-items-center tw-gap-4">
                                    <div
                                        class="tw-inline-flex tw-items-center tw-justify-center tw-w-10 tw-h-10 tw-text-green-500 tw-bg-green-100 tw-rounded-full sm:tw-w-12 sm:tw-h-12 tw-shrink-0 dashboard-icon">
                                        <svg aria-hidden="true" class="tw-w-6 tw-h-6" xmlns="http://www.w3.org/2000/svg"
                                            viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none"
                                            stroke-linecap="round" stroke-linejoin="round">
                        ...
                        <div
                            ...
                            <div class="tw-p-4 sm:tw-p-5">
                                <div class="tw-flex tw-items-center tw-gap-4">
                                    <div
                                        class="tw-inline-flex tw-items-center tw-justify-center tw-w-10 tw-h-10 tw-text-red-500 tw-bg-red-100 tw-rounded-full sm:tw-w-12 sm:tw-h-12 shrink-0 dashboard-icon">
                                        <svg aria-hidden="true" class="tw-w-6 tw-h-6" xmlns="http://www.w3.org/2000/svg"
                                            viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none"
                                            stroke-linecap="round" stroke-linejoin="round">
                                            <path stroke="none" d="M0 0h24v24H0z" fill="none" />
        ...
        @if (auth()->user()->can('dashboard.data'))
            @if ($is_admin)
                <div class="tw-relative sell-summary hide">
                    <div class="tw-absolute tw-inset-0 tw-grid" aria-hidden="true">
                        <div
                            class="tw-bg-gradient-to-r tw-from-@if (!empty(session('business.theme_color'))) {{ session('business.theme_color') }}@else{{ 'primary' }} @endif-800 tw-to-@if (!empty(session('business.theme_color'))) {{ session('business.theme_color') }}@else{{ 'primary' }} @endif-900">
                        </div>
                        <div
                            class="tw-bg-gradient-to-r tw-from-@if (!empty(session('business.theme_color'))) {{ session('business.theme_color') }}@else{{ 'primary' }} @endif-800 tw-to-@if (!empty(session('business.theme_color'))) {{ session('business.theme_color') }}@else{{ 'primary' }} @endif-900 xl:tw-bg-none xl:tw-bg-gray-100">
                        </div>
                    </div>
                        ...
                        <div
                            ...
                            <div
                                ...
                                <div class="tw-p-4 sm:tw-p-5">
                                    <div class="tw-flex tw-items-center tw-gap-4">
                                        <div
                                            class="tw-inline-flex tw-items-center tw-justify-center tw-w-10 tw-h-10 tw-rounded-full sm:tw-w-12 sm:tw-h-12 shrink-0 tw-bg-sky-100 tw-text-sky-500 dashboard-icon">
    ...
    @if (auth()->user()->can('dashboard.data'))
        <div class="tw-px-5 tw-py-6 order-summary">
        ...
        <div class="tw-px-5 tw-py-6 sell-summary hide">
            <div class="tw-grid tw-grid-cols-1 tw-gap-4 sm:tw-gap-5 lg:tw-grid-cols-2">
                @if (auth()->user()->can('sell.view') || auth()->user()->can('direct_sell.view'))
                    @if (!empty($all_locations))
                        <div
...
@section('javascript')
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            order_table = $('#ready_to_deliver_table').DataTable({
                processing: true,
                serverSide: true,
                fixedHeader: false,
                aaSorting: [
                ...
                "ajax": {
                    ...
                    "data": function(d) {
                        d.delivery_status = 'ready_to_deliver';
                        if ($('#ready_to_deliver_location').length > 0) {
                            d.location_id = $('#ready_to_deliver_location').val();
                        }
                    }
                },
                columns: [{
            ...
            $('#ready_to_deliver_location').change(function() {
                order_table.ajax.reload();
            });

            $(document).on("change", "#tailoring_status", function() {
                const val = $(this).val();
                if (val == "sell") {
                    $('.order-summary').addClass('hide');
                    $('.sell-summary').removeClass('hide');
                } else {
                    $('.order-summary').removeClass('hide');
                    $('.sell-summary').addClass('hide');
                }
            });
        });
    </script>
