...
class TransactionUtil extends Util
{
    ...
    public function createOrUpdateClothSellLines($transaction, $cloths, $location_id, $return_deleted = false, $status_before = null, $extra_line_parameters = [], $uf_data = true)
    {
        ...
        foreach ($cloths as $cloth) {
            ...
            if (! empty($cloths['transaction_sell_lines_id'])) {
                ...
                //update or create modifiers for existing sell lines
                if ($this->isModuleEnabled('modifiers')) {
                    if (! empty($cloth['modifier'])) {
                        foreach ($cloth['modifier'] as $key => $value) {
                            if (! empty($product['modifier_sell_line_id'][$key])) {
                                $edit_modifier = TransactionSellLine::find($cloth['modifier_sell_line_id'][$key]);
                                $edit_modifier->quantity = isset($cloth['modifier_quantity'][$key]) ? $cloth['modifier_quantity'][$key] : 1;
                                $modifiers_formatted[] = $edit_modifier;
                                //Dont delete modifier sell line if exists
                                $edit_ids[] = $cloth['modifier_sell_line_id'][$key];
                            } else {
                                if (! empty($cloth['modifier_price'][$key])) {
                                    $this_price = $uf_data ? $this->num_uf($cloth['modifier_price'][$key]) : $cloth['modifier_price'][$key];
                                    $modifier_quantity = isset($cloth['modifier_quantity'][$key]) ? $cloth['modifier_quantity'][$key] : 1;
                                    $modifiers_formatted[] = new TransactionSellLine([
                                        'cloth_id' => $cloth['modifier_set_id'][$key],
                                        // 'variation_id' => $value,
                                        'quantity' => $modifier_quantity,
                                        'unit_price_before_discount' => $this_price,
                                        'unit_price' => $this_price,
                                        'unit_price_inc_tax' => $this_price,
                                        'parent_sell_line_id' => $cloth['transaction_sell_lines_id'],
                                        'children_type' => 'modifier',
                                    ]);
                                }
                            }
                ...
                //calculate unit price and unit price before discount
                $uf_unit_price = $uf_data ? $this->num_uf($cloth['unit_price']) : $cloth['unit_price'];
                $unit_price_before_discount = $uf_unit_price / $multiplier;
                $unit_price = $unit_price_before_discount;
                $uf_quantity = $uf_data ? $this->num_uf($cloth['quantity']) : $cloth['quantity'];
                $uf_item_tax = 0;
                $uf_unit_price_inc_tax = $uf_data ? $this->num_uf($cloth['unit_price_inc_tax']) : $cloth['unit_price_inc_tax'];

                $line_discount_amount = 0;
                if (! empty($cloth['line_discount_amount'])) {
                    $line_discount_amount = $uf_data ? $this->num_uf($cloth['line_discount_amount']) : $cloth['line_discount_amount'];

                    if ($cloth['line_discount_type'] == 'fixed') {
                        $line_discount_amount = $line_discount_amount / $multiplier;
                    }
                }
                ...
                $line = [
                    ...
                    'unit_price_before_discount' => $unit_price_before_discount,
                    'unit_price' => $unit_price,
                    'line_discount_type' => $cloth['line_discount_type'],
                    'line_discount_amount' => $line_discount_amount,
                    'item_tax' => $uf_item_tax / $multiplier,
                    'tax_id' => null,
                    'unit_price_inc_tax' => $uf_unit_price_inc_tax / $multiplier,
    ...
    public function editClothSellLine($cloth, $location_id, $status_before, $multiplier = 1, $uf_data = true)
    {
        ...
        $old_qty = $sell_line->quantity;
        $edit_ids[] = $cloth['transaction_sell_lines_id'];

        // Adjust quantity
        if ($status_before != 'draft') {
            $new_qty = $this->num_uf($cloth['quantity']) * $multiplier;
            $difference = $sell_line->quantity - $new_qty;

            // $this->adjustClothQuantity($location_id, $cloth['cloth_id'], $difference);
        }
        ...
        if (!empty($cloth['line_discount_type']) && $cloth['line_discount_amount']) {
            $discount_amount = $uf_data ? $this->num_uf($cloth['line_discount_amount']) : $cloth['line_discount_amount'];
            if ($cloth['line_discount_type'] == 'fixed') {
                $unit_price = $unit_price_before_discount - ($discount_amount / $multiplier);
            } elseif ($cloth['line_discount_type'] == 'percentage') {
                $unit_price = ((100 - $discount_amount) * $unit_price_before_discount) / 100;
            }
        }
        ...
        $sell_line->fill([
            'cloth_id' => $cloth['cloth_id'],
            'quantity' => $uf_data ? $this->num_uf($cloth['quantity']) * $multiplier : $cloth['quantity'] * $multiplier,
            'unit_price_before_discount' => $unit_price_before_discount,
            'unit_price' => $unit_price,
        ...
        // Add any additional logic here for warranties, combo cloths, or style/measurement customizations
    ...
    public function deleteSellLines($transaction_line_ids, $location_id, $adjust_qty = true)
    {
        if (! empty($transaction_line_ids)) {
            ...
            foreach ($sell_lines as $line) {
                if ($adjust_qty && !empty($line->product_id)) {
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        $output['lines'] = [];
        $total_exempt = 0;
        if (in_array($transaction_type, ['sell', 'order', 'sales_order'])) {
        ...
        if (($transaction_type == 'sell' || $transaction_type == 'order') && $transaction->status == 'final') {
    ...
    protected function _receiptDetailsSellLines($lines, $il, $business_details)
    {
        ...
        foreach ($lines as $line) {
            $product = !empty($line->cloth_id) ? $line->cloth : $line->product;
            $variation = $line->variations;
            $product_variation = empty($line->cloth_id) ? $line->variations->product_variation : null;
            $unit = empty($line->cloth_id) ? $line->product->unit : null;
            $brand = empty($line->cloth_id) ? $line->product->brand : null;
            $cat = empty($line->cloth_id) ? $line->product->category : null;
            ...
            $line_array = [
                ...
                'name' => !empty($line->cloth_id) ? $product->cloth_name : $product->name,
                'line_type' => !empty($line->cloth_id) ? 'order' : null,
                ...
                'variation_id' => empty($variation) ? '' : $variation->id,
            ];
    ...
    public function getOrderTotals($business_id, $start_date = null, $end_date = null, $location_id = null, $created_by = null, $permitted_locations = null)
    {
        $query = Transaction::where('transactions.business_id', $business_id)
    ...
    public function getOrdersCurrentFy($business_id, $start, $end)
    {
        $query = Transaction::leftjoin('transactions as SR', function ($join) {
            $join->on('SR.return_parent_id', '=', 'transactions.id')
                ->where('SR.type', 'sell_return');
    ...
    public function deleteSale($business_id, $transaction_id)
    {
        ...
        $transaction = Transaction::where('id', $transaction_id)
            ->where('business_id', $business_id)
            ->whereIn('type', ['sell', 'order', 'sales_order'])
        ...
        $output = [
            'success' => true,
            'msg' => __($is_order ? 'tailoring.order_delete' : 'lang_v1.sale_delete_success'),
        ];
    ...
    public function getListSells($business_id, $sale_type = 'sell')
    {
        $sells = Transaction::leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
            ...
            ->select(
                ...
                DB::raw('SUM(COALESCE(tsl.quantity, 0)) as total_items'),
        ...
        if ($sale_type == 'sell' || $sale_type == 'order') {
    ...
    public function getLedgerDetails($contact_id, $start, $end, $format = 'format_1', $location_id = null, $line_details = false)
    {
        ...
        foreach ($transactions as $transaction) {
            ...
            $temp_array = [
                ...
                'ref_no' => in_array($transaction->type, ['sell', 'order', 'sell_return']) ? $transaction->invoice_no : $transaction->ref_no,
                'type' => $transaction_types[$transaction->type],
                'location' => $transaction->location->name ?? '',
                'payment_status' => (! in_array($transaction->type, ['ledger_discount']) && !empty($transaction->payment_status)) ? __('lang_v1.' . $transaction->payment_status) : '',
                'total' => '',
                'payment_method' => '',
                'debit' => in_array($transaction->type, ['sell', 'order', 'purchase_return']) || ($transaction->sub_type == 'purchase_discount') ? $transaction->final_total : '',
        ...
        $order_invoice_sum = $transactions->where('type', 'order')->sum('final_total');
        ...
        $order_invoice_paid_sum = ! empty($payments) ? $payments->where('transaction_type', 'order')->sum('amount') : 0;
        $total_invoice_paid += $order_invoice_paid_sum;
        ...
        $total_order_invoice = $order_invoice_sum;
        ...
        $curr_due = $total_invoice + $total_order_invoice + $total_purchase - $total_transactions_paid + $beginning_balance + $opening_balance_due;
        ...
        $overall_transaction_sums = $this->__transactionQuery($contact_id, null, null, $location_id)
            ->select(
                DB::raw("SUM(IF(type = 'purchase', final_total, 0)) as total_purchase"),
                DB::raw("SUM(IF((type = 'sell') AND status = 'final', final_total, 0)) as total_invoice"),
                DB::raw("SUM(IF((type = 'order') AND status = 'final', final_total, 0)) as total_overall_order_invoice"),
                DB::raw("SUM(IF(type = 'sell_return', final_total, 0)) as total_sell_return"),
                DB::raw("SUM(IF(type = 'purchase_return', final_total, 0)) as total_purchase_return"),
                DB::raw("SUM(IF(type = 'opening_balance', final_total, 0)) as total_opening_balance"),
                DB::raw("SUM(IF(type = 'ledger_discount', final_total, 0)) as total_ledger_discount")
        ...
        $overall_total_order_paid = $overall_payments->where('transaction_type', 'order')->sum('amount');
        ...
        $total_overall_paid_customer = $overall_total_invoice_paid - $overall_total_sell_return_paid + $overall_total_ob_paid + $overall_total_order_paid; //Add '+ $overall_total_advance_payment'

        $total_overall_paid_supplier = $overall_total_purchase_paid - $overall_total_purchase_return_paid;
        $overall_due = $total_overall_invoice + $overall_transaction_sums->total_overall_order_invoice + $total_overall_purchase - $total_overall_paid_customer - $total_overall_paid_supplier - $overall_ledger_discount;
        ...
        $output = [
            ...
            'total_order_invoice' => $total_order_invoice,
            ...
            'all_order_total_invoice' => $overall_transaction_sums->total_overall_order_invoice,
