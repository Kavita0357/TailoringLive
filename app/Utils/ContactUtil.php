    ...
    public function getContactQuery($business_id, $type, $contact_ids = [])
    {
        ...
        if (in_array($type, ['customer', 'both'])) {
            $query->addSelect([
                ...
                DB::raw("SUM(IF(t.type = 'order' AND t.status = 'final', 1, 0)) as total_orders"),
                DB::raw("MAX(CASE WHEN t.type = 'order' THEN transaction_date ELSE NULL END) as last_order_date"),
            ]);
        }
