...
use App\TaxRate;
use App\ClothOrder;
use App\ClothOrderPayment;
use App\ClothOrderLine;
use App\ClothOrderLinesPurchaseLines;
use App\Variation;
...
class ClothOrderUtil extends Util
{
    /**
    ...
    public function createOrUpdateSellLines($transaction, $products, $location_id, $return_deleted = false, $status_before = null, $extra_line_parameters = [], $uf_data = true)
    {
        ...
        foreach ($products as $product) {
            ...
            if (! empty($product['transaction_sell_lines_id'])) {
                    ...
                    if (! empty($product['modifier'])) {
                        foreach ($product['modifier'] as $key => $value) {
                            if (! empty($product['modifier_sell_line_id'][$key])) {
                                $edit_modifier = ClothOrderLine::find($product['modifier_sell_line_id'][$key]);
                                ...
                                if (! empty($product['modifier_price'][$key])) {
                                    ...
                                    $modifiers_formatted[] = new ClothOrderLine([
                ...
                \Log::emergency(json_encode($line));

                $lines_formatted[] = new ClothOrderLine($line);

                \Log::emergency(json_encode($lines_formatted));
        ...
        if (! is_object($transaction)) {
            $transaction = ClothOrder::findOrFail($transaction);
        }

        //Delete the products removed and increment product stock.
        $deleted_lines = [];
        if (! empty($edit_ids)) {
            $deleted_lines = ClothOrderLine::where('transaction_id', $transaction->id)
                ->whereNotIn('id', $edit_ids)
                ->select('id')->get()->toArray();
            $combo_delete_lines = ClothOrderLine::whereIn('parent_sell_line_id', $deleted_lines)->where('children_type', 'combo')->select('id')->get()->toArray();
                ...
                foreach ($lines_formatted as $key => $value) {
                    if (! empty($modifiers_array[$key])) {
                        foreach ($modifiers_array[$key] as $modifier) {
                            ...
                            $modifiers_formatted[] = new ClothOrderLine($modifier);
                        }
                    }
                }
            }
    ...
    private function updateSalesOrderLine($so_line_id, $new_qty, $old_qty = 0)
    {
        ...
        if (! empty($so_line_id) && ! empty($diff)) {
            $so_line = ClothOrderLine::find($so_line_id);
    ...
    private function __makeLinesForComboProduct($combo_items, $parent_sell_line)
    {
        ...
        foreach ($combo_items as $value) {
            ...
            $combo_lines[] = new ClothOrderLine([
    ...
    public function editSellLine($product, $location_id, $status_before, $multiplier = 1, $uf_data = true)
    {
        ...
        $sell_line = ClothOrderLine::with(['product', 'warranties'])
    ...
    public function deleteSellLines($transaction_line_ids, $location_id, $adjust_qty = true)
    {
        if (! empty($transaction_line_ids)) {
            $sell_lines = ClothOrderLine::whereIn('id', $transaction_line_ids)
                ->get();
            ...
            ClothOrderLine::whereIn('so_line_id', $transaction_line_ids)
                ->update(['so_line_id' => null]);

            ClothOrderLine::whereIn('id', $transaction_line_ids)
                ->delete();
        }
    }
        ...
        foreach ($payments as $payment) {
            ...
            if (! empty($payment['payment_id'])) {
                ...
                if ($payment_amount != 0) {
                    ...
                    $payments_formatted[] = new ClothOrderPayment($payment_data);
    ...
    public function editPaymentLine($payment, $transaction = null, $uf_data = true)
    {
        ...
        $tp = ClothOrderPayment::where('id', $payment_id)
            ->first();
    ...
    public function updateCashDenominations($payment, $all_denominations)
    {
            ...
            foreach ($all_denominations as $key => $value) {
                if (! empty($value)) {
                    CashDenomination::updateOrCreate(
                        ['business_id' => $payment->business_id, 'model_id' => $payment->id, 'model_type' => \App\ClothOrderPayment::class, 'amount' => $key],
                        ['total_count' => $value]
                    );
                    $denominations[] = $key;
                }
        ...
        CashDenomination::where('business_id', $payment->business_id)
            ->where('model_type', \App\ClothOrderPayment::class)
    ...
    {
        $payment_lines = ClothOrderPayment::where('transaction_id', $transaction_id)->with(['denominations'])
            ->get()->toArray();
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        $transaction = ClothOrder::find($transaction_id);
        ...
        if ($transaction_type == 'sell_return') {
            ...
            $output['parent_invoice_no'] = ClothOrder::find($transaction->return_parent_id)->invoice_no;
        ...
        if (in_array($transaction_type, ['sell', 'sales_order'])) {
            ...
            $parent_sell = ClothOrder::find($transaction->return_parent_id);
                ...
                if (! empty($payments)) {
                    foreach ($payments as $value) {
                        ...
                        if ($value['method'] == 'cash') {
                            ...
                            $output['payments'][] =
                                [
                                    'method' => $method . (! empty($value['card_transaction_number']) ? (', ClothOrder Number:' . $value['card_transaction_number']) : ''),
                                    'amount' => $this->num_f($value['amount'], $show_currency, $business_details),
                                    'date' => $this->format_date($value['paid_on'], false, $business_details),
                                ];
        ...
        if (! empty($transaction->sales_order_ids)) {
            $sale_orders = ClothOrder::where('type', 'sales_order')
    ...
    {
        $products = ClothOrder::join('purchase_lines as pl', 'transactions.id', '=', 'pl.transaction_id')
    ...
    {
        $query = ClothOrderPayment::join('transactions as t', 't.id', 'transaction_payments.transaction_id')
    ...
    {
        $query = ClothOrder::where('transactions.business_id', $business_id)
    ...
    public function getInputTax($business_id, $start_date = null, $end_date = null, $location_id = null, $contact_id = null)
    {
        ...
        $query1 = ClothOrder::where('transactions.business_id', $business_id)
        ...
        $query2 = ClothOrder::where('transactions.business_id', $business_id)
    ...
    {
        $query = ClothOrder::leftjoin('transactions as SR', function ($join) {
    ...
    public function getExpenseReport(
        ...
        $query = ClothOrder::leftjoin('expense_categories AS ec', 'transactions.expense_category_id', '=', 'ec.id')
            ->where('transactions.business_id', $business_id)
    ...
    {
        $total_paid = ClothOrderPayment::where('transaction_id', $transaction_id)
    ...
    public function calculatePaymentStatus($transaction_id, $final_amount = null)
    {
        ...
        if (is_null($final_amount)) {
            $final_amount = ClothOrder::find($transaction_id)->final_total;
        }
    ...
    public function payAtOnce($parent_payment, $type)
    {
        ...
        $due_transactions = ClothOrder::where('contact_id', $parent_payment->payment_for)
            ->whereIn('type', $types)
            ->where('payment_status', '!=', 'paid')
        ...
        if ($due_transactions->count()) {
            ...
            if (! empty($tranaction_payments)) {
                ClothOrderPayment::insert($tranaction_payments);
            }
        }
    ...
    public function mapPurchaseSell($business, $transaction_lines, $mapping_type = 'purchase', $check_expiry = true, $purchase_line_id = null)
    {
        ...
        foreach ($transaction_lines as $line) {
            ...
            $query = ClothOrder::join('purchase_lines AS PL', 'transactions.id', '=', 'PL.transaction_id')
            ...
            if (! empty($purchase_adjustment_map)) {
                ClothOrderLinesPurchaseLines::insert($purchase_adjustment_map);
            }
            if (! empty($purchase_sell_map)) {
                ClothOrderLinesPurchaseLines::insert($purchase_sell_map);
            }
        }
    }
    ...
    public function adjustMappingPurchaseSell(
        ...
        if ($status_before == 'final' && $transaction->status == 'draft') {
            ...
            $sell_purchases = ClothOrder::join('transaction_sell_lines AS SL', 'transactions.id', '=', 'SL.transaction_id')
            ...
            if (! empty($deleted_line_ids)) {
                $deleted_sell_purchases = ClothOrderLinesPurchaseLines::whereIn('sell_line_id', $deleted_line_ids)
            ...
            if (! empty($sell_purchases)) {
                ...
                //Delete the lines.
                ClothOrderLinesPurchaseLines::whereIn('id', $sell_purchase_ids)
                    ->delete();
            }
            ...
            foreach ($sell_purchases as $line) {
                if (empty($line->slpl_id)) {
                    ...
                    $total_sold_entry = ClothOrderLinesPurchaseLines::where('sell_line_id', $line->id)
    ...
    {
        $sell_purchase_line = ClothOrderLinesPurchaseLines::where('sell_line_id', $sell_line_id)
            ->orderBy('id', 'desc')
            ->get();
    ...
    public function mapPurchaseQuantityForDeleteStockAdjustment($line_ids)
    {
        ...
        $map_line = ClothOrderLinesPurchaseLines::whereIn('stock_adjustment_line_id', $line_ids)
            ->orderBy('id', 'desc')
            ->get();
        ...
        ClothOrderLinesPurchaseLines::whereIn('stock_adjustment_line_id', $line_ids)
            ->delete();
    ...
    public function adjustMappingPurchaseSellAfterEditingPurchase($before_status, $transaction, $delete_purchase_lines)
    {
        if ($before_status == 'received' && $transaction->status == 'received') {
            ...
            $purchase_lines = ClothOrder::join(
        ...
        foreach ($purchase_lines as $purchase_line) {
            ...
            $tspl = ClothOrderLinesPurchaseLines::where('purchase_line_id', $purchase_line['id'])
                ->leftjoin(
            ...
            foreach ($tspl as $row) {
                if ($row->quantity <= $extra_sold) {
                    ...
                    ClothOrderLinesPurchaseLines::where('id', $row->tslpl_id)->delete();
                } else {
                    if (! empty($row->sell_line_id)) {
                        $sell_lines[] = (object) [
                            'id' => $row->sell_line_id,
                    ...
                    ClothOrderLinesPurchaseLines::where('id', $row->tslpl_id)->update(['quantity' => $row->quantity - $extra_sold]);
    ...
    {
        if (! is_object($transaction)) {
            $transaction = ClothOrder::find($transaction);
        }
    ...
    public function getTotalSellCommission($business_id, $start_date = null, $end_date = null, $location_id = null, $commission_agent = null)
    {
        ...
        $query = ClothOrderLine::leftjoin('transactions as t', 'transaction_sell_lines.transaction_id', '=', 't.id')
            ->where('t.business_id', $business_id)
            ->where('t.type', 'sell')
    ...
    public function updateQuantitySoldFromSellLine($sell_line, $new_quantity, $old_quantity, $uf_number = true)
    {
        ...
        if ($qty_difference != 0) {
            ...
            $sell_line_purchase_lines = ClothOrderLinesPurchaseLines::where('sell_line_id', $sell_line->id)->get();
    ...
    {
        return ClothOrder::where('return_parent_id', $transacion_id)->exists();
    }
    ...
    public function isLotUsed($transaction)
    {
        foreach ($transaction->purchase_lines as $purchase_line) {
            $exists = ClothOrderLine::where('lot_no_line_id', $purchase_line->id)->exists();
            if ($exists) {
                return true;
            }
        }
    ...
    public function createRecurringInvoice($transaction, $is_draft = false)
    {
        ...
        $recurring_invoice = ClothOrder::create($data);
    ...
    {
        $query = ClothOrderLine::join('transactions as sale', 'transaction_sell_lines.transaction_id', '=', 'sale.id')
    ...
    {
        $purchases = ClothOrder::leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
            ->join(
    ...
    {
        $expenses = ClothOrder::leftJoin('expense_categories AS ec', 'transactions.expense_category_id', '=', 'ec.id')
    ...
    {
        $sells = ClothOrder::leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
    ...
    public function getLedgerDetails($contact_id, $start, $end, $format = 'format_1', $location_id = null, $line_details = false)
    {
        ...
        $transaction_types = ClothOrder::transactionTypes();
        $ledger = [];
    ...
    {
        $business_id = request()->session()->get('user.business_id');
        $transaction_type_keys = array_keys(ClothOrder::transactionTypes());

        $query = ClothOrder::where('transactions.contact_id', $contact_id)
            ->where('transactions.business_id', $business_id)
    ...
    private function __paymentQuery($contact_id, $start, $end = null, $location_id = null)
    {
        ...
        $query = ClothOrderPayment::leftJoin(
            'transactions as t',
            'transaction_payments.transaction_id',
            '=',
            't.id'
    ...
    public function getProfitLossDetails($business_id, $location_id, $start_date, $end_date, $user_id = null, $permitted_locations = null)
    {
        ...
        $sales_by_subtype = ClothOrder::where('business_id', $business_id)
            ->where('type', 'sell')
            ->where('status', 'final');
        if (! empty($start_date) && ! empty($end_date)) {
    ...
    public function createRecurringExpense($transaction)
    {
        ...
        $recurring_expense = ClothOrder::create($data);
    ...
    public function createExpense($request, $business_id, $user_id, $format_data = true)
    {
        ...
        $transaction = ClothOrder::create($transaction_data);
    ...
    public function payContact($request, $format_data = true)
    {
        ...
        $parent_payment = ClothOrderPayment::create($inputs);
    ...
    public function addSellReturn($input, $business_id, $user_id, $uf_number = true)
    {
        ...
        if (empty($sell_return)) {
            ...
            $sell_return = ClothOrder::create($sell_return_data);
    ...
    public function updatePurchaseOrderStatus($purchase_order_ids = [])
    {
        foreach ($purchase_order_ids as $purchase_order_id) {
            $purchase_order = ClothOrder::with(['purchase_lines'])->find($purchase_order_id);
    ...
    public function getPdfContentsForGivenTransaction($business_id, $transaction_id)
    {
        ...
        //Get transaction
        $transaction = ClothOrder::findOrFail($transaction_id);
    ...
    public function updateSalesOrderStatus($sales_order_ids = [])
    {
        foreach ($sales_order_ids as $sales_order_id) {
            $sales_order = ClothOrder::with(['sell_lines'])->find($sales_order_id);
    ...
    {
        $totals = ClothOrder::where('business_id', $business_id)
    ...
    {
        $unique_sources = ClothOrder::where('business_id', $business_id)
            ->where('type', 'sell')
            ->select('source')
            ->groupBy('source')
            ->get();
    ...
    public function getPurchaseOrderPdf($business_id, $transaction_id)
    {
        ...
        $purchase = ClothOrder::where('business_id', $business_id)
