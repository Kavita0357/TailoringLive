...
class ModuleUtil extends Util
{
    ...
    public function countOrders($business_id, $start_dt, $end_dt)
    {
        $query = Transaction::where('business_id', $business_id)
            ->where('type', 'order')
            ->where('status', 'final');
    ...
    public function countCloths($business_id, $start_dt, $end_dt)
    {
        $query = Cloth::where('business_id', $business_id);
    ...
    public function isQuotaAvailable($type, $business_id, $total_rows = 0)
    {
        ...
        if ($is_available) {
            ...
            if ($type == 'locations') {
                ...
                $max_allowed = isset($package->package_details['cloths_count']) ? $package->package_details['cloths_count'] : 0;

                if ($max_allowed == 0) {
                    return true;
                } else {
                    $count = $this->countCloths($business_id, $start_dt, $end_dt);
                    if ($count >= $max_allowed) {
                        return false;
                    }
                }
                ...
                \Log::info($type);
                $max_allowed = isset($package->package_details['orders_count']) ? $package->package_details['orders_count'] : 0;
                \Log::info(json_encode($package));
                if ($max_allowed == 0) {
                    return true;
                } else {
                    $count = $this->countOrders($business_id, $start_dt, $end_dt);
                    if ($count >= $max_allowed) {
                        return false;
                    }
                }
    ...
    public function quotaExpiredResponse($type, $business_id, $redirect_url = null)
    {
        if ($type == 'locations') {
            ...
            $response_array = [
                'success' => 0,
                'msg' => __('superadmin::lang.max_cloths'),
            ];
            ...
            $response_array = [
                'success' => 0,
                'msg' => __('superadmin::lang.max_orders'),
            ];
