        ...
        if ($request->ajax()) {
            $contacts = Contact::where('contacts.business_id', $business_id)
                ...
                ->select(
                    ...
                    DB::raw("SUM(IF(t.type = 'order' AND t.status = 'final', (SELECT SUM(IF(is_return = 1,-1*amount,amount)) FROM transaction_payments WHERE transaction_payments.transaction_id=t.id), 0)) as order_invoice_received"),
            ...
            return Datatables::of($contacts)
                ...
                ->editColumn(
                    'total_order_invoice',
                    '<span class="total_order_invoice" data-orig-value="{{$total_order_invoice}}">@format_currency($total_order_invoice)</span>'
                )
                ...
                ->addColumn(
                    'order_due',
                    '<span class="total_order_due" data-orig-value="{{$total_order_invoice - $order_invoice_received}}">@format_currency($total_order_invoice - $order_invoice_received)</span>'
                )
                ->addColumn(
                    'opening_balance_due',
