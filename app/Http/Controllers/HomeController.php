...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        //get all orders
        $orders_this_fy = $this->transactionUtil->getOrdersCurrentFy($business_id, $least_30_days, $fy['end']);
        ...
        for ($i = 29; $i >= 0; $i--) {
            ...
            $total_order_on_date = $orders_this_fy->where('date', $date)->sum('total_sells');

            if (! empty($total_order_on_date)) {
                $all_order_values[] = (float) $total_order_on_date;
            } else {
                $all_order_values[] = 0;
            }
        }
        ...
        foreach ($all_locations as $loc_id => $loc_name) {
            $values = [];
            foreach ($dates as $date) {
                $total_order_on_date_location = $orders_this_fy->where('date', $date)->where('location_id', $loc_id)->sum('total_sells');

                if (! empty($total_order_on_date_location)) {
                    $values[] = (float) $total_order_on_date_location;
                } else {
                    $values[] = 0;
                }
            }
            $location_orders[$loc_id]['loc_label'] = $loc_name;
            $location_orders[$loc_id]['values'] = $values;
        }

        $orders_chart_1 = new CommonChart;

        $orders_chart_1->labels($labels)
            ->options($this->__chartOptions(__(
                'home.total_orders',
                ['currency' => $currency->code]
            )));
        ...
        if (! empty($location_orders)) {
            foreach ($location_orders as $location_sell) {
                $orders_chart_1->dataset($location_sell['loc_label'], 'line', $location_sell['values']);
            }
        }

        if (count($all_locations) > 1) {
            $orders_chart_1->dataset(__('report.all_locations'), 'line', $all_order_values);
        }
        ...
        do {
            ...
            $total_sell_in_month_year = $orders_this_fy->where('yearmonth', $month_year)->sum('total_sells');
        ...
        $fy_orders_by_location_data = [];

        foreach ($all_locations as $loc_id => $loc_name) {
            $values_data = [];
            foreach ($fy_months as $month) {
                $total_order_in_month_year_location = $orders_this_fy->where('yearmonth', $month)->where('location_id', $loc_id)->sum('total_sells');

                if (! empty($total_order_in_month_year_location)) {
                    $values_data[] = (float) $total_order_in_month_year_location;
                } else {
                    $values_data[] = 0;
                }
            }
            $fy_orders_by_location_data[$loc_id]['loc_label'] = $loc_name;
            $fy_orders_by_location_data[$loc_id]['values'] = $values_data;
        }

        $orders_chart_2 = new CommonChart;
        $orders_chart_2->labels($labels)
            ->options($this->__chartOptions(__(
                'home.total_orders',
                ['currency' => $currency->code]
            )));
        if (! empty($fy_orders_by_location_data)) {
            foreach ($fy_orders_by_location_data as $location_sell) {
                $orders_chart_2->dataset($location_sell['loc_label'], 'line', $location_sell['values']);
            }
        }
        if (count($all_locations) > 1) {
            $orders_chart_2->dataset(__('report.all_locations'), 'line', $values);
        }
    ...
    public function getTotals()
    {
        if (request()->ajax()) {
            ...
            $tailoring_status = request()->tailoring_status;
            $business_id = request()->session()->get('user.business_id');
            ...
            $order_details = $this->transactionUtil->getOrderTotals($business_id, $start, $end, $location_id, $created_by, null, true);
            ...
            $output['total_order'] = $total_order_inc_tax;
            $output['order_invoice_due'] = $order_details['invoice_due'] - $total_ledger_discount['total_sell_discount'];

            //NET = TOTAL SALES - INVOICE DUE - EXPENSE
            $output['net'] = $output['total_sell'] - $output['invoice_due'] - $output['total_expense'];

            $output['tailoring_status'] = $tailoring_status;
    ...
    /**
     * Retrieves payment dues for the orders.
     *
     * @return \Illuminate\Http\Response
     */
    public function getOrdersPaymentDues()
    {
        if (request()->ajax()) {
            $business_id = request()->session()->get('user.business_id');
            $today = \Carbon::now()->format('Y-m-d H:i:s');
