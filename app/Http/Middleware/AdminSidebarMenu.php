        ...
        Menu::create('admin-sidebar-menu', function ($menu) {
            ...
            if (in_array('tailoring', $enabled_modules)) {
                ...
                $menu->dropdown(
                    ...
                    function ($sub) use ($common_settings) {
                        ...
                        $sub->url(
                            route('cloth_orders.index'),
                            __('tailoring.all_orders'),
                            ['icon' => '', 'active' => request()->segment(1) == 'cloth-orders' && request()->segment(2) == null && empty(request()->get('delivery_status'))]
                        );
                        $sub->url(
                            route('cloth_orders.create'),
                        ...
                        /* $sub->url(
                            ...
                            ['icon' => '', 'active' => request()->segment(1) == 'cloth-orders' && request()->get('status') == 'draft']
                        ); */
                        $sub->url(
                            route('cloth_orders.drafts'),
                            __('lang_v1.list_drafts'),
                            ['icon' => '', 'active' => request()->segment(1) == 'cloth-orders' && request()->segment(2) == 'drafts']
                        );

                        $sub->url(
                            route('cloth_orders.quotations'),
                            __('lang_v1.list_quotations'),
                            ['icon' => '', 'active' => request()->segment(1) == 'cloth-orders' && (request()->segment(2) == 'quotations' || request()->get('status') == 'quotation')]
                        );

                        $sub->url(
                            route('cloth_orders.index', ['delivery_status' => 'preparing']),
                            __('tailoring.preparing_orders'),
                            ['icon' => '', 'active' => request()->get('delivery_status') == 'preparing']
                        );

                        $sub->url(
                            route('cloth_orders.index', ['delivery_status' => 'ready_to_deliver']),
                            __('tailoring.ready_to_deliver'),
                            ['icon' => '', 'active' => request()->get('delivery_status') == 'ready_to_deliver']
                        );

                        $sub->url(
                            route('cloth_orders.index', ['delivery_status' => 'delivered']),
                            __('tailoring.delivered_orders'),
                            ['icon' => '', 'active' => request()->get('delivery_status') == 'delivered']
                        );
                    },
                    [
                        'icon' => '<svg aria-hidden="true" class="tw-size-5 tw-shrink-0" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
            ...
            if ($is_admin || auth()->user()->hasAnyPermission(['sell.view', 'sell.create', 'direct_sell.access', 'view_own_sell_only', 'view_commission_agent_sell', 'access_shipping', 'access_own_shipping', 'access_commission_agent_shipping', 'access_sell_return', 'direct_sell.view', 'direct_sell.update', 'access_own_sell_return'])) {
                $menu->dropdown(
                    ...
                    function ($sub) use ($enabled_modules, $is_admin, $pos_settings) {
                        ...
                        if (in_array('add_sale', $enabled_modules) && auth()->user()->can('direct_sell.access')) {
                            $sub->url(
                                route('sells.create', ['status' => 'draft']),
                                __('lang_v1.add_draft'),
                                ['icon' => '', 'active' => request()->segment(1) == 'sells' && request()->get('status') == 'draft']
                            );
                        }
                        ...
                        if (in_array('add_sale', $enabled_modules) && auth()->user()->can('direct_sell.access')) {
                            $sub->url(
                                route('sells.create', ['status' => 'quotation']),
                                __('lang_v1.add_quotation'),
                                ['icon' => '', 'active' => request()->segment(1) == 'sells' && request()->get('status') == 'quotation']
                            );
                        }
                        if (in_array('add_sale', $enabled_modules) && ($is_admin || auth()->user()->hasAnyPermission(['quotation.view_all', 'quotation.view_own']))) {
                            $sub->url(
                                route('sells.quotations'),
                                __('lang_v1.list_quotations'),
                                ['icon' => '', 'active' => request()->segment(1) == 'sells' && request()->segment(2) == 'quotations' && request()->get('status') == null]
                            );
                        }
            ...
            $menu->dropdown(
                ...
                function ($sub) {
                    ...
                    $sub->url(
                        action([\App\Http\Controllers\MessagingController::class, 'create']),
                        'Send Bulk SMS',
                        [
                            'icon' => '',
                            'active' => request()->segment(1) == 'messaging' && request()->segment(2) == 'create'
                        ]
                    );
            ...
            if (auth()->user()->can('backup')) {
                $menu->url(action([\App\Http\Controllers\BackUpController::class, 'index']), __('lang_v1.backup'), [
                    'icon' => '<svg aria-hidden="true" class="tw-size-5 tw-shrink-0" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
                <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
            ...
            if (auth()->user()->can('manage_modules')) {
                $menu->url(action([\App\Http\Controllers\Install\ModulesController::class, 'index']), __('lang_v1.modules'), [
                    'icon' => '<svg aria-hidden="true" class="tw-size-5 tw-shrink-0" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
              <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
              <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
